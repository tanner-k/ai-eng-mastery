...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For scaling inference, pay special attention to the assumptions behind latency budgets, batching, caching, model replicas, accelerator use, and degradation strategies.
...
The plan should turn "design a scalable inference plan for synthetic burst and steady-state traffic" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
