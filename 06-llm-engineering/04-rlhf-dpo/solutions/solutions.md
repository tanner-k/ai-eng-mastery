...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For rlhf and dpo, pay special attention to the assumptions behind preference data, reward modeling, policy optimization, and direct preference objectives.
...
The plan should turn "fit a toy pairwise preference model and reason about chosen/rejected examples" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
