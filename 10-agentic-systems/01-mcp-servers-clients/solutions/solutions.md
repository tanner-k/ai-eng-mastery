# MCP Servers and Clients — Solutions
...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For mcp servers and clients, pay special attention to the assumptions behind tool contracts, resource exposure, client/server boundaries, and safe tool invocation.
...
The plan should turn "design a toy MCP server contract for local project search" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
