...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For langgraph, pay special attention to the assumptions behind state graphs, nodes, edges, checkpoints, control flow, and human-in-the-loop recovery.
...
The plan should turn "design a state-machine agent workflow with retry and review paths" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
