...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For graph databases, pay special attention to the assumptions behind nodes, relationships, property graphs, queries, indexing, and transactional graph storage.
...
The plan should turn "model synthetic entities and write query plans conceptually" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
