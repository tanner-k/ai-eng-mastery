...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For experiment tracking, pay special attention to the assumptions behind run metadata, parameters, metrics, artifacts, lineage, and reproducibility.
...
The plan should turn "design a lightweight experiment tracker over synthetic training runs" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
