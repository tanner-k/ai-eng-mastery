...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For model serving, pay special attention to the assumptions behind online inference APIs, batching, latency budgets, model loading, rollout, and rollback.
...
The plan should turn "design a toy model-serving contract with canary and fallback behavior" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
