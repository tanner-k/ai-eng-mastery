...
16. **Q: Why is learning-rate warmup important for adaptive optimizers like Adam?**
    **A:** With bias correction in place, Adam's early steps are actually well-scaled: at t=1 with β₂=0.999, v̂ = v/(1−β₂¹) = (1−β₂)g²/(1−β₂) = g², so √v̂ = |g| and the step is approximately lr — there is no divide-by-near-zero blowup. The real problem is that the moment estimates at step 1 are based on a single, potentially very noisy gradient. When the initial gradient is large and unrepresentative, the update variance is high: the optimizer can lurch in an unlucky direction and land far from a productive region of the loss surface. Warmup ramps the effective learning rate from ~0 to the target value over the first several hundred to few thousand steps, reducing the magnitude of early updates while the moment estimates accumulate enough history to become reliable. This is especially critical in transformer training, where the loss landscape at initialization is poorly conditioned and early gradient estimates are particularly noisy.
...
- **Warmup and the first few steps of Adam.** Many candidates can explain Adam's update formula but cannot explain why warmup is necessary. The key is to reason about what bias correction actually does: at t=1 with β₂=0.999, v̂ = v/(1−0.999¹) = (1−0.999)g²/(0.001) = g², so √v̂ = |g| and the step is well-scaled (≈ lr). There is no divide-by-near-zero blowup from bias correction. The real motivation for warmup is update variance: with only one gradient in the history, the moment estimates are very noisy and a full-size learning rate can send the optimizer on a large, unlucky step. Warmup keeps η small while the estimates accumulate enough history to become reliable.
