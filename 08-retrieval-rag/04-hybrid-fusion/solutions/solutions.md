...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For hybrid fusion, pay special attention to the assumptions behind sparse/dense score fusion, normalization, reciprocal rank fusion, and recall tradeoffs.
...
The plan should turn "combine BM25 and dense rankings for ambiguous synthetic queries" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
