...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For semantic retrieval, pay special attention to the assumptions behind dense embeddings, similarity search, recall, indexing, and embedding drift.
...
The plan should turn "embed toy passages and compare nearest neighbors" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
