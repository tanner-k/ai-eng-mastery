...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For vector databases, pay special attention to the assumptions behind ANN indexes, metadata filters, persistence, recall/latency, and operational constraints.
...
The plan should turn "compare exact and approximate nearest-neighbor behavior conceptually" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
