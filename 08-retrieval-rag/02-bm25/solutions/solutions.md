...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For bm25, pay special attention to the assumptions behind term frequency, inverse document frequency, length normalization, and sparse retrieval.
...
The plan should turn "rank synthetic documents with a hand-computed BM25 score" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
