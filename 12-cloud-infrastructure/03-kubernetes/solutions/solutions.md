...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For kubernetes, pay special attention to the assumptions behind pods, deployments, services, config, scheduling, rollout, and health management.
...
The plan should turn "map a model-serving workload onto Kubernetes deployment primitives" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
