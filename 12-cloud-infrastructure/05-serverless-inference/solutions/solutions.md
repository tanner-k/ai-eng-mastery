# Serverless Inference — Solutions
...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For serverless inference, pay special attention to the assumptions behind scale-to-zero, cold starts, concurrency, timeout limits, and stateless inference contracts.
...
The plan should turn "design a serverless inference path for bursty synthetic traffic" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
