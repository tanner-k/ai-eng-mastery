...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For q-learning, pay special attention to the assumptions behind temporal-difference targets, bootstrapping, off-policy control, and exploration.
...
The plan should turn "train tabular Q-learning in a small deterministic environment" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
