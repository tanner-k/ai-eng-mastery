...
A complete answer should name the data model, allowed inputs, expected outputs, state or configuration, and invariants. For policy gradients, pay special attention to the assumptions behind stochastic policies, score-function gradients, baselines, variance, and advantage estimates.
...
The plan should turn "implement a toy REINFORCE update over sampled trajectories" into a runnable lab with deterministic toy inputs, explicit assertions, and a short written interpretation. It should avoid hidden datasets or external services unless they are optional extensions.
